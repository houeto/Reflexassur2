<?php

namespace MainBundle\Controller;
use MainBundle\Entity\Garantir;
use MainBundle\Form\GarantirType;

use Symfony\Bundle\FrameworkBundle\Controller\Controller;

class GarantirController extends Controller
{
    public function listeGarantirAction()
    {
        //Déclaration d'une varibale pour récupérer le modèle
        $em = $this->getDoctrine()->getManager();
        
        //Déclaration d'un objet de type Garantir
        $newGarantir = new Garantir();
        //Création du formulaire en passant l'objet Garantir en paramètre
        $form = $this->createForm(new GarantirType(), $newGarantir);
        //Requête pour vérifier si c'est un get ou post
        $request = $this->get("request");
        if ($request->getMethod() == 'POST') { //Post
            $form->bind($request);
            //Vérification si le modèle est valide
            if ($form->isValid()) {
                //Vérification de doublon
                //Doublon sur le id de Garantir
                $Garantir_doublon = $em ->getRepository("MainBundle:Garantir")->findByid($newGarantir->getId());
                if($Garantir_doublon){
                    $this->get('session')->getFlashBag()->add('error', 'Cet enregistrement existe déjà.');
                    return $this->redirect($this->generateUrl('garantir'));
                }                
                    //Instruction pour l'enregistrement
                    $em->persist($newGarantir);
                    //Mise à jour des informations
                    $em->flush();
                    //Affichage de l'information de l'enregistrement
                    $this->get('session')->getFlashBag()->add('success', 'Enregistrement ajouté avec succès');
                    return $this->redirect($this->generateUrl('garantir'));   
            }
        }
        //Requête pour sélectionner la liste des garanties
        $garantir = $em->getRepository("MainBundle:Garantir")->findAll();
        //Retourne la vue
        return $this->render('MainBundle:Actions:Garantir.html.twig', array(
                'form' => $form->createView(),
                'garantir' => $garantir
            ));    
        
    }    
    
    public function modifierGarantirAction($id) {
        //Déclaration d'une variable pour récupérer le modèle
        $em = $this->getDoctrine()->getManager();
        //recherche de l'objet à modifier par l'id
        $garantir = $em->getRepository("MainBundle:Garantir")->find($id);
        
        //Création du formulaire en passant l'objet à modifier en paramètre
        $form = $this->createForm(new GarantirType(), $garantir);
        //Vérification de la méthode du formulaire si c'est un get ou un post
        $request = $this->get("request");
        if ($request->getMethod() == 'POST') {//Post
            //Récupération du contenu du formulaire
            $form->bind($request);
            //Vérification si le formulaire est valide
            if ($form->isValid()) {
                //Instruction pour modifier
                $em->persist($garantir);
                //Mise à jour
                $em->flush();
                //Affichage d'un message
                $this->get('session')->getFlashBag()->add('success', 'Enregistrement modifié avec succès');
                //Rédirection
                return $this->redirect($this->generateUrl('garantir'));
            }
        }
        //Requête pour récupérer la liste des polices d'assurance
        $garantir = $em->getRepository("MainBundle:Garantir")->findAll();
        return $this->render('MainBundle:Actions:Garantir.html.twig', array(
                    'form' => $form->createView(),
                    'modification' => true,
                    'garantir' => $garantir
        ));
    }    
    
    public function supprimerGarantirAction($id) {
        //Récupération du modèle
        $em = $this->getDoctrine()->getManager();
        //Recherche de l'objet à supprimer
        $garantir = $em->getRepository("MainBundle:Garantir")->find($id);
        //Tester le résultat
        if ($garantir) {
            //Suppression
            $em->remove($garantir);
            //Mise à jour des informations
            $em->flush();
            //Affichage de message
            $this->get('session')->getFlashBag()->add('success', 'Enregistrement supprimé avec succès');
            //Rédirection 
            return $this->redirect($this->generateUrl('garantir'));
        } else {//Objet non trouvé
            //Affichage de message
            $this->get('session')->getFlashBag()->add('error', 'Cet enregistrement n\'existe plus');
            //Rédirection
            return $this->redirect($this->generateUrl('garantir'));
        }
    }
}
